...
# USAGE ---
# This order will spend some amount of time fleeing directly away from the target
...
@export var min_offset: float = 25
@export var max_offset: float = 150
...
func plot_order() -> void:
	...
	var order_finish = ((order_origin - order_target).normalized() * speed * ORDER_DURATION) + order_origin
...
func execute_order(delta: float) -> bool:
	...
	character.sprite.rotation = lerp_angle(character.sprite.rotation, character.position.angle_to_point(order_finish) - PI, 3 * PI * delta)
